...
class ConditionalModel(nn.Module):
    def __init__(self, config):
        ...
        resolution = config.dataset.image_size
        ...
        block_in = None
        for i_level in range(self.num_resolutions): # in range(4)

            block = nn.ModuleList()
            attn = nn.ModuleList()

            block_in = ch * in_ch_mult[i_level] # 64 * 1
            block_out = ch * ch_mult[i_level] # 64 * 1

            for i_block in range(self.num_res_blocks): # in range(1)
                block.append(ResnetBlock(in_channels=block_in,
                                         out_channels=block_out,
                                         temb_channels=self.temb_ch,
                                         dropout=dropout))
                block_in = block_out
                if curr_res in attn_resolutions: # if image resolution is downsampled to eg.16
